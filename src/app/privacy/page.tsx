export default function PrivacyPage() {
  return (
    <div className="max-w-4xl mx-auto px-4 py-12">
      <h1 className="text-3xl font-bold mb-8">Privacy Policy</h1>
      <div className="prose prose-gray">
        <p className="mb-4">Last updated: March 2024</p>
        <h2 className="text-xl font-semibold mt-8 mb-4">1. Information We Collect</h2>
        <p className="mb-4">We collect name, email, and form data for processing assistance.</p>
        <h2 className="text-xl font-semibold mt-8 mb-4">2. Data Security</h2>
        <p className="mb-4">Your data is stored securely with industry-standard encryption.</p>
        <h2 className="text-xl font-semibold mt-8 mb-4">3. Contact</h2>
        <p className="mb-4">For privacy questions, contact us at privacy@formguide.ai</p>
      </div>
    </div>
  )
}
